...
def test_create_or_get_pull_request_reuses_existing(monkeypatch):
    calls = []
    ...
    result = asyncio.run(github_app.create_or_get_draft_pull_request(
        REPOSITORY_URL,
...
def test_create_pull_request_is_always_draft_and_not_maintainer_mutable(monkeypatch):
    calls = []
...
def test_remote_branch_head_is_read_from_exact_ref(monkeypatch):
    seen = {}
    ...
    monkeypatch.setattr(github_app.httpx, "AsyncClient", FakeClient)
    sha = asyncio.run(github_app.get_remote_branch_head(
        REPOSITORY_URL,
